...
async def update_variables_in_file(logger: Logger, account, updates: dict):
    wallet = account.public_key
    file_path = f'./data/accounts/{wallet}.txt'
    os.makedirs(os.path.dirname(file_path), exist_ok=True)

    if os.path.isfile(file_path):
        existing_data = await read_account(wallet)
    else:
        existing_data = {}
...
async def read_account(wallet: str) -> dict[str, str]:
    variables = {}
    file_path = f'./data/accounts/{wallet}.txt'
    if os.path.isfile(file_path):
        with open(f'./data/accounts/{wallet}.txt', 'r') as file:
            for line in file:
